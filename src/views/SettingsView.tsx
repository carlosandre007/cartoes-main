...
export const SettingsView: React.FC = () => {
  const { isDarkMode, toggleDarkMode, isPwaInstallable, installPwa, bankAccounts, transactions } = useFinancial();
  const { catalog: categories, addCategory, renameCategory, removeCategory } = useCategories(transactions.map((tx) => tx.categoria));
  ...
  const handleRenameCategory = (category: string) => {
    ...
    if (name && name !== category) renameCategory(category, name);
  };

  const handleRemoveCategory = (category: string) => {
    const used = transactions.some((tx) => tx.categoria === category);
    const message = used
      ? `A categoria "${category}" está em lançamentos existentes. Removê-la da lista não altera o histórico. Continuar?`
      : `Remover a categoria "${category}" da lista?`;
    ...
    <div className="p-6 space-y-6 text-zinc-100 font-sans">
      ...
      <div className="p-6 rounded-2xl bg-zinc-950/90 border border-amber-500/20 shadow-xl space-y-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-bold text-zinc-100 flex items-center gap-2"><Tags className="w-4 h-4 text-amber-400" /> Categorias globais</h3>
            <p className="text-[11px] text-zinc-400 mt-1">Uma única lista para receitas, despesas, custos fixos e cartões.</p>
          </div>
          <button type="button" onClick={handleAddCategory} className="px-3 py-2 rounded-xl bg-amber-500 text-zinc-950 text-xs font-black flex items-center gap-1"><Plus className="w-4 h-4" /> Nova categoria</button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {categories.map((category) => (
            <div key={category} className="px-3 py-2.5 rounded-xl bg-zinc-900/70 border border-zinc-800 flex items-center justify-between gap-2">
              <span className="text-xs font-semibold text-zinc-200 truncate">{category}</span>
              <div className="flex shrink-0">
                <button type="button" onClick={() => handleRenameCategory(category)} title="Renomear categoria" className="p-1.5 text-zinc-400 hover:text-amber-300"><Edit2 className="w-3.5 h-3.5" /></button>
                <button type="button" onClick={() => handleRemoveCategory(category)} title="Remover categoria" className="p-1.5 text-zinc-500 hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          ))}
        </div>
